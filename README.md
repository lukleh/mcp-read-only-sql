...
- **Host-key verification**: Both implementations verify the bastion's host key the way `ssh` does. `ssh_tunnel.host_key_checking` takes the OpenSSH `StrictHostKeyChecking` values and defaults to `accept-new`: a bastion is recorded on first use and refused if its key changes. `yes` refuses bastions not already recorded, and `no` restores the previous behaviour: every key is trusted and none is recorded. Known keys are read from `/etc/ssh/ssh_known_hosts` and `~/.ssh/known_hosts`, and new ones are recorded in the latter, unless `ssh_tunnel.known_hosts_file` names another file to read and record in. The Python implementation appends new keys in OpenSSH's own line format, so both implementations and `ssh` share one record. A changed key surfaces as an `SSH:` error naming host key verification; remove the stale line from the file once you have confirmed the new key.
