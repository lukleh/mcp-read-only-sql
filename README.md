...
- **Host-key verification**: Both implementations verify the bastion's host key the way `ssh` does. `ssh_tunnel.host_key_checking` takes the OpenSSH `StrictHostKeyChecking` values and defaults to `accept-new`: a bastion is recorded on first use and refused if its key changes. `yes` refuses bastions not already recorded, and `no` restores the previous trust-everything behaviour. Keys are read from and recorded in `~/.ssh/known_hosts` unless `ssh_tunnel.known_hosts_file` points elsewhere; the Python implementation appends new keys in OpenSSH's own line format, so both implementations and `ssh` share one record. A changed key surfaces as an `SSH:` error naming host key verification; remove the stale line from the file once you have confirmed the new key.
